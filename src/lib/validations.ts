...
export const driverOnboardingSchema = z.object({
  ...
  registrationYear: z.number().min(1990).max(new Date().getFullYear() + 1, "Invalid year").optional(),
  vehicleNumber: z.string().min(6, "Vehicle number is required"),
  yearsOfExperience: z.number().min(0).max(40).optional(),
  seatCount: z.number().min(1).max(8).optional(),
  ac: z.boolean(),
